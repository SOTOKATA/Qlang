...
public class Interpreter(Dictionary<string, string> stringDictionary)
{
    ...
    public void Execute(ProgramNode program)
    {
        ...
        if (_functions.TryGetValue("main", out var function))
            ExecuteFunction(function, []);
        else
            throw new Exception("No 'main' function found!");
    }

    private void ExecuteFunction(FunctionNode function, List<object> arguments)
    {
    ...
    private void ExecuteMethodCall(MethodCallNode call)
    {
        ...
        foreach (QClass qClass in QNamespace.GetClassList())
        {
            ...
            function.Execute(call.Arguments.ConvertAll(EvaluateExpression).ToArray());
        }
    }
